...
#MUNIC_RES indica o c√≥digo do munic√≠pio de resid√™ncia do entrante
#A base com PIBs per capita cont√©m os mesmo c√≥digos, por√©m com um d√≠gito a mais
...
## Baixando dados de Maio, Junho e Julho para constru√ß√£o da base final
...
#Adicionando o c√≥digo da regi√£o de sa√∫de
...
base_PIB <- base_PIB %>% 
  rename(codigo_microrregiao = "CÛdigo da Microrregi„o")

base_PIB <- base_PIB %>% 
  rename(codigo_estado = "CÛdigo da Grande Regi„o") %>% 
  rename(estado = "Nome da Grande Regi„o") %>% 
  rename(codigo_UF = "CÛdigo da Unidade da FederaÁ„o") %>% 
  rename(UF = "Sigla da Unidade da FederaÁ„o") %>% 
  rename(nome_UF = "Nome da Unidade da FederaÁ„o") %>% 
  rename(codigo_municipio = "CÛdigo do MunicÌpio") %>% 
  rename(nome_municipio = "Nome do MunicÌpio") %>% 
  rename(regiao_metropolitana = "Regi„o Metropolitana") %>% 
  rename(codigo_mesorregiao = "CÛdigo da Mesorregi„o") %>% 
